/*******************************************************************
*	Name              :	Mcycle_Driver
*	Description       : Driver class for Mcycle Quote-NB LOB
*	Author            : ATS  
...
import com.alliance.page.mcycle.spi.Mcycle_ClassInvoker;

public class Mcycle_Driver extends Mcycle_ClassInvoker {	
	...
	public void Mcycle (TestContainer testContainer) throws Exception 
    //public void Mcycle (TestName testName, Device device) throws Exception 
    {		
		try{ 
			...
			AlliancePageKeywords.strPolicyReorderFlag= changeURLData.getData("PolicyReorder");
			AlliancePageKeywords.strPolicyReorderDB= changeURLData.getData("PolicyReorderDB");

			//Load test data from SQL DB
			CustomLoadDataElements.LoadData("MCYCLE",strDB,tcID, driver);

			//Impersonate, Main menu and Name/Address Page
			startStep(testName, "Impersonate Menu Page", "Input Data in Impersonate Menu Page");
			impersonateMenu(tcID,changeURLData,"Mcycle");
			...
			/*AlliancePageKeywords absAlliancePage = new AlliancePageKeywords();
			absAlliancePage.failureScreenprint();
			ReportTestFailure(e);   */     	
		}	
	}
}
